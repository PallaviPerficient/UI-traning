...
const Edit = () => {
    ...
    useEffect(() => {
        async function getStudent() {
            try {
                const student = await axios.get(`http://localhost:3004/student/${id}`)
    ...
    return (
        <>
            <Box textAlign="center" p={2} className={classes.headingColor} mb={2}>
                <Typography variant="h2">Update Data</Typography>
            </Box>
                ...
                <Grid item md={6} xs={12}>
                    ...
                    <form>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                <TextField autoComplete="number" name="number" variant="outlined" required fullWidth id="number" label="Number" value={student.number} onChange={e => onTextFieldChange(e)} />
                            </Grid>
                        </Grid>
                        <Box m={3}>
